...
### JPA를 사용해야 하는 이유
#### 📌생산성
JPA를 사용하면 자바 컬렉션에 객체를 저장하듯이 JPA에 저장할 객체만 전달하면 된다.

#### 📌유지보수
SQL을 사용하면 코드를 수정해야할 때, 많은 코드를 변경해야 했다.

반면 JPA를 사용하면 수정해야할 코드가 매우 줄어든다.

또한 JPA가 패러다임의 불일치 문제를 해결해주면서 객체지향 언어가 가진 장점을 활용하여 유연하고 유지보수하기 좋은 도메인 모델을 설계할 수 있다.

#### 📌패러다임의 불일치 해결
JPA는 상속, 연관관계, 객체 그래프 탐색, 비교하기 등의 패러다임의 불일치 문제를 해결해준다.

#### 📌성능
JPA는 애플리케이션과 데이터베이스 사이에서 다양한 성능 최적화 기회를 제공한다.

#### 📌데이터 접근 추상화와 벤더 독립성
관계형 데이터베이스는 같은 기능도 벤더마다 사용법이 다른 경우가 많은데, JPA는 애플리케이션과 데이터베이스 사이에 추상화된 데이터 접근 계층을 제공하여 애플리케이션이 특정 데이터베이스에 종속되지 않게 한다.
